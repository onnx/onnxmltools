...
def normalize_tensor(scope, tensor_name, normalized_tensor_name, container):
    container.add_node('LpNormalization', tensor_name, normalized_tensor_name,
                       name=scope.get_unique_operator_name('LpNormalization'), p=2, axis=1)
...
def convert_keras_dot(scope, operator, container):
    ...
    if op.normalize:
        for tensor_name in operator.input_full_names:
            ...
            normalize_tensor(scope, tensor_name, normalized_tensor_name, container)
    ...
    container.add_node('Mul', normalized_input_names, intermediate_tensor_name,
