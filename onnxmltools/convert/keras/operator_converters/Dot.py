...
def convert_keras_dot(scope, operator, container):
    ...
    if op.normalize:
        for tensor_name in operator.input_full_names:
            ...
            apply_normalization(scope, tensor_name, normalized_tensor_name, container, operator_name=scope.get_unique_operator_name(name_prefix + "LpNormalization"))
            normalized_input_names.append(normalized_tensor_name)
    else:
        normalized_input_names = operator.input_full_names

    intermediate_tensor_name = scope.get_unique_variable_name(name_prefix + 'elementwise_product')
    apply_mul(scope, normalized_input_names, intermediate_tensor_name, container, operator_name=scope.get_unique_operator_name(name_prefix + "Mul"))

    container.add_node('ReduceSum', intermediate_tensor_name, operator.output_full_names,
                       name=scope.get_unique_operator_name(name_prefix + 'ReduceSum'), axes=[1], keepdims=1)
