...
def convert_keras_pooling_core(scope, operator, container, is_global, n_dims,
    ...
    output_node_name = scope.get_unique_variable_name('pooling_for_reshape') if is_global else operator.outputs[0].full_name
    ...
    if channels_first:
        ...
        container.add_node(op_type_prefix + onnx_op_type, adjusted_pooling_input,
                           output_node_name, op_version=op_version, **attrs)
    else:
        ...
        apply_transpose(scope, pooling_output_name, output_node_name, container, perm=output_perm_axes)

    if is_global:
        apply_reshape(scope, output_node_name,
