...
class SoftmaxLayerConverter:

    @staticmethod
    def validate(cm_node):
        try:
            utils._check_has_attr(cm_node, 'softmax')
    ...
    def convert(context, cm_node, inputs, outputs):
        nb = NodeBuilder(context, 'Softmax')
        nb.extend_inputs(inputs)
        nb.extend_outputs(outputs)
        return nb.make_node()


registration.register_nn_converter('softmax', SoftmaxLayerConverter)
