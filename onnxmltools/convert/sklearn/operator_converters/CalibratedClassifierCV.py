...
class Oper:
    def __init__(self, model, inputs, op_type, label_name, prob_name, prob_tensor_name):
        self.raw_operator = model
        self.inputs = inputs
        self.type = op_type
        self.outputs = [Output(label_name), Output(prob_name), Output(prob_tensor_name)]

def convert_calibrated_classifier_base_estimator(scope, operator, container, flag=False, model=None):
    ...
    prob_name = scope.get_unique_variable_name('probability')
    df_name = scope.get_unique_variable_name('probability_tensor')
    this_operator = Oper(base_model, operator.inputs, op_type, label_name, prob_name, df_name)
...
def convert_sklearn_calibrated_classifier_cv(scope, operator, container):
    ...
    for clf in op.calibrated_classifiers_:
        prob_scores_name.append(convert_calibrated_classifier_base_estimator(scope, operator, container, flag=True, model=clf))

    container.add_node('Sum', [s for s in prob_scores_name],
                       add_result_name, name=scope.get_unique_operator_name('Sum'), op_version=7)
    apply_div(scope, [add_result_name, clf_length_name], operator.outputs[2].full_name, container, broadcast=1)
    class_prob_name = operator.outputs[2].full_name
