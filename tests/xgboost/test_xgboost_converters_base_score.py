...
class TestXGBoostModelsBaseScore(unittest.TestCase):
    ...
    def test_xgbclassifier_multiclass_base_score_in_onnx(self):
        ...
        self.assertIsNotNone(
            tree_ensemble_node, "TreeEnsembleClassifier node not found"
        )
        ...
        self.assertIsNotNone(
            base_values, "base_values attribute not found in ONNX model"
        )
        self.assertEqual(
            len(base_values),
            3,
            "base_values should have 3 elements for 3-class problem",
        )
