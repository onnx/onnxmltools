...
class TestSparkmlImputer(SparkMlTestCase):
    ...
    @unittest.skipIf(
        True, reason="Name:'Split' Status Message: Cannot split using values in 'split"
    )
    @unittest.skipIf(sys.version_info < (3, 8), reason="pickle fails on python 3.7")
    ...
    def _imputer_test_multi(self):
        ...
        expected = (
            predicted.select("out_a", "out_b").toPandas().values.astype(numpy.float32)
        )
        ...
        output, output_shapes = run_onnx_model(
            ["out_a", "out_b"], data_np, onnx_model_path
        )
