...
class TestNeuralNetworkLayerConverter(unittest.TestCase):
    ...
    def test_unary_function_converter(self):
        ...
        builder = NeuralNetworkBuilder(input, output)
        builder.add_unary(
            name="Unary1", input_name="input", output_name="mid1", mode="abs"
        )
        builder.add_unary(
            name="Unary2", input_name="mid1", output_name="mid2", mode="sqrt"
        )
        builder.add_unary(
            name="Unary3", input_name="mid2", output_name="mid3", mode="rsqrt"
        )
        builder.add_unary(
            name="Unary4", input_name="mid3", output_name="mid4", mode="inverse"
        )
        builder.add_unary(
            name="Unary5", input_name="mid4", output_name="mid5", mode="power", alpha=2
        )
        builder.add_unary(
            name="Unary6", input_name="mid5", output_name="mid6", mode="exp"
        )
        builder.add_unary(
            name="Unary7", input_name="mid6", output_name="mid7", mode="log"
        )
        builder.add_unary(
            name="Unary8", input_name="mid7", output_name="output", mode="threshold"
        )
        model_onnx = convert_coreml(builder.spec, target_opset=TARGET_OPSET)
        self.assertTrue(model_onnx is not None)
    ...
    def test_load_constant_converter(self):
        ...
        inputs = [("input", datatypes.Array(*input_dim))]
        outputs = [
            ("const", datatypes.Array(*shape)),
            ("output", datatypes.Array(*input_dim)),
        ]
        builder = NeuralNetworkBuilder(inputs, outputs)
        builder.add_load_constant(
            name="LoadConstant", output_name="const", constant_value=value, shape=shape
        )
        builder.add_permute(
            name="Permute", input_name="input", output_name="output", dim=(0, 1, 2, 3)
        )
        model_onnx = convert_coreml(builder.spec, target_opset=TARGET_OPSET)
        self.assertTrue(model_onnx is not None)
    ...
    def test_flatten_converter(self):
        ...
        builder = NeuralNetworkBuilder(inputs, outputs)
        builder.add_flatten(
            name="Flatten", input_name="input", output_name="output", mode=1
        )
        model_onnx = convert_coreml(builder.spec, target_opset=TARGET_OPSET)
        self.assertTrue(model_onnx is not None)
    ...
    def test_permute_converter(self):
        ...
        builder = NeuralNetworkBuilder(inputs, outputs)
        builder.add_permute(
            name="Permute", input_name="input", output_name="output", dim=(0, 2, 3, 1)
        )
        model_onnx = convert_coreml(builder.spec, target_opset=TARGET_OPSET)
        self.assertTrue(model_onnx is not None)
    ...
    def test_split_converter(self):
        ...
        inputs = [("input", datatypes.Array(*input_dim))]
        outputs = [
            ("output1", datatypes.Array(*output_dim)),
            ("output2", datatypes.Array(*output_dim)),
        ]
        builder = NeuralNetworkBuilder(inputs, outputs)
        builder.add_split(
            name="Split", input_name="input", output_names=["output1", "output2"]
        )
        model_onnx = convert_coreml(builder.spec, target_opset=TARGET_OPSET)
        self.assertTrue(model_onnx is not None)
    ...
    def test_sequence_repeat_converter(self):
        ...
        builder.add_sequence_repeat(
            name="Repeat", input_name="input", output_name="output", nrep=3
        )
        model_onnx = convert_coreml(builder.spec, target_opset=TARGET_OPSET)
        self.assertTrue(model_onnx is not None)
    ...
    def test_image_input_type_converter(self):
        ...
        for coreml_colorspace, onnx_colorspace in (
            ("RGB", "Rgb8"),
            ("BGR", "Bgr8"),
            ("GRAYSCALE", "Gray8"),
        ):
