...
class TestGLMClassifierConverter(unittest.TestCase):
    ...
    def test_model_knn_classifier_binary_class(self):
        ...
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(np.atleast_2d(X[0]).astype(numpy.float32), model, model_onnx, basename="SklearnKNeighborsClassifierBinary")
    ...
    def test_model_knn_classifier_multi_class(self):
        ...
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(np.atleast_2d(X[0]).astype(numpy.float32), model, model_onnx, basename="SklearnKNeighborsClassifierMulti")
