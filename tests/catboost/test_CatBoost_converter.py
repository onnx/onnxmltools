...
class TestCatBoost(unittest.TestCase):
    @unittest.skipIf(
        catboost is None or sklearn is None, reason="catboost not imported"
    )
    def test_catboost_regressor(self):
        ...
        catboost_model = catboost.CatBoostRegressor(
            task_type="CPU", loss_function="RMSE", n_estimators=10, verbose=0
        )
        dump_single_regression(catboost_model)

        catboost_model.fit(X.astype(numpy.float32), y)
        catboost_onnx = convert_catboost(
            catboost_model, name="CatBoostRegression", doc_string="test regression"
        )
    ...
    def test_catboost_bin_classifier(self):
        ...
        if pv.Version(".".join(onnxruntime.__version__.split(".")[:2])) >= pv.Version(
            ...
            warnings.warn(
                "Converted CatBoost models for binary classification "
                "work with onnxruntime version 1.3.0 or "
                "a newer one"
            )
    ...
    def test_catboost_multi_classifier(self):
        X, y = make_classification(
            n_samples=10, n_informative=8, n_classes=3, random_state=0
        )
        catboost_model = catboost.CatBoostClassifier(
            task_type="CPU", loss_function="MultiClass", n_estimators=100, verbose=0
        )
        ...
        catboost_onnx = convert_catboost(
            catboost_model,
            name="CatBoostMultiClassification",
